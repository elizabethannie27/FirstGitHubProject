      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.length
          ? `<ul>${details.participants
              .map(
                (participant) =>
                  `<li data-email="${participant}"><span class="participant-name">${participant}</span><button type="button" class="participant-delete" aria-label="Remove ${participant}">✕</button></li>`
              )
              .join("")}</ul>`
        ...
        activityCard.querySelectorAll(".participant-delete").forEach((button) => {
          button.addEventListener("click", async () => {
            const participantEmail = button.closest("li").dataset.email;
